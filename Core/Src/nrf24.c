...
void NRF24_csn(int state)
{
	if (state)
		...
		HAL_GPIO_WritePin(GPIOA, nrf24_CSN_PIN, GPIO_PIN_RESET);
}
//2. Chip Enable
...
{
	NRF24_write_registerN(REG_RX_ADDR_P0, address, size);
	NRF24_write_registerN(REG_TX_ADDR, address, size);
...
void NRF24_openReadingPipe(uint8_t number, uint8_t *address, uint8_t size)
{
	...
	if (number <= 6)
	{
		if (number < 2)
		{
			...
			NRF24_write_registerN(NRF24_ADDR_REGS[number], address, size);
		}
		else
		{
			NRF24_write_registerN(NRF24_ADDR_REGS[number], address, size);
...
void printRadioSettings(void)
{
	...
	printf("RX_Pipe2 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	HAL_UART_Transmit(&nrf24_huart, (uint8_t *)uartTxBuf, strlen(uartTxBuf), 10);

	NRF24_read_registerN(0x0A + 3, pipeAddrs, 1);
	printf("RX_Pipe3 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	HAL_UART_Transmit(&nrf24_huart, (uint8_t *)uartTxBuf, strlen(uartTxBuf), 10);

	NRF24_read_registerN(0x0A + 4, pipeAddrs, 1);
	printf("RX_Pipe4 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	HAL_UART_Transmit(&nrf24_huart, (uint8_t *)uartTxBuf, strlen(uartTxBuf), 10);

	NRF24_read_registerN(0x0A + 5, pipeAddrs, 1);
	printf("RX_Pipe5 Addrs:\r\n		xx,xx,xx,xx,%02X  \r\n", pipeAddrs[0]);
	HAL_UART_Transmit(&nrf24_huart, (uint8_t *)uartTxBuf, strlen(uartTxBuf), 10);
